...
csvsql --query "select Host,Method,avg(Duration) as 'mean',sqrt(avg(pow(Duration,2))-pow(avg(Duration),2)) as 'stdev' from 'importoptim' group by Host,Method order by mean" importoptim.csv | csvlook
